 ...
 * next; each test is free to call up() as many times as it needs.
 */
...
it('up() swaps the narrow key for the wide one when the narrow key is the one present', function (): void {
    Schema::table($this->table, fn (Blueprint $table) => $table->dropUnique('cf_code_entity_section_unique'));
    Schema::table($this->table, fn (Blueprint $table) => $table->unique(['code', 'entity_type'], 'custom_fields_code_entity_type_unique'));
