...
- **Keep `composer test:types` clean** at PHPStan level 6, with no baseline; an ignore is accepted only for a finding that differs between the CI dependency legs, and it must name the drift in a comment
