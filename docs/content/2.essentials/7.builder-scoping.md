...
Passing `[]` (the default) means "no scope", so existing call sites that never call
...
field rows. That is a data-corrupting outcome that has nothing to do with whether resolution
...
Return `null` to leave the uniqueness check global. That is the default, backward-compatible
...
the same instance), but a closure that hands back a different instance (for example
