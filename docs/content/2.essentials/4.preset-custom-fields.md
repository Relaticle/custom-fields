...
Each entry is either the option name on its own, or an array carrying the name plus the
option settings beside it (`color`, and `category` for a single-choice field):

```php
->options([
    'Draft',
    ['name' => 'Active', 'category' => OptionCategory::Started, 'color' => '#16a34a'],
    ['name' => 'Discontinued', 'category' => OptionCategory::Cancelled],
])
```

An array without a `name` throws `InvalidArgumentException`. See
[Option Categories](/essentials/option-categories) for what a category means.
...
`lookupType()` creates a one-way relationship definition with the field as its single slot;
the field itself stores no target. Cardinality comes from `allow_multiple` on the field data
(many-to-many when it is true, many-to-one otherwise), or pass one explicitly:

```php
->lookupType(User::class, RelationshipCardinality::ManyToMany)
```

The ends of a relationship are locked once it exists, so `update()` rejects a `lookup_type`
key. Delete the field and add it again to point it somewhere else.
