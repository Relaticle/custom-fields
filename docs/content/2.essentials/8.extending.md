...
so one callback can behave differently per builder. `record` is the persisted model
when the builder was given one; it is null when the builder was given a model class
or an unsaved instance, which is the case for tables, exporters and create forms.
