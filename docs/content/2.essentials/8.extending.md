...
`kind` tells the callback which builder is asking, so one callback can behave
differently per surface. A filter is only ever handed `ResolutionKind::Table`,
`Infolist` or `Exporter`; the enum also names `Form` and `Importer` for the builders
that run no filters, so a branch on either of those never fires. `record` is the
persisted model when the builder was given one; it is null when the builder was given
a model class or an unsaved instance, which is the case for tables, exporters and
create forms.
