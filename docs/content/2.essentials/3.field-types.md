...
**`requiresRelationship()`** -- Replaces the user-defined options UI with the record configuration: the target entity, the cardinality, and an optional paired field. The field renders one end of a relationship definition and stores links rather than static option values. Import/export treats these as entity references. Currently used by the Record field type.
