...
| Flavor | Renders |
|---|---|
| `polished` (default) | The package's own Blade views, `custom-fields::flavors.polished.<surface>`. |
| `native` | Stock Filament markup, from the same Livewire classes. |
...
Every other screen renders stock Filament in both flavors. An unknown flavor, or a key that
is not one of the five, throws rather than falling back silently, so a typo in a published
config fails loudly instead of quietly reverting a surface.
