...
or a key that is not one of the five, throws when the package boots rather than falling back
silently, so a typo in a published config fails on the first request instead of quietly
reverting a surface.
