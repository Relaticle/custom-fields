...
| `record-chips` | Linked records shown on a record. |
| `record-picker` | The search-and-select panel that links them. |
...
| `SYSTEM_RELATIONSHIPS` | On | Gates the relationship migrations and the lookup_type drop; a record field with a definition reads, writes, and deletes its links either way |
