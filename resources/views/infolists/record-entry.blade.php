@php
    ...
    $chipsView = $state['chipsView'] ?? null;
@endphp

<x-dynamic-component :component="$getEntryWrapperView()" :entry="$entry">
    @if ($chipsView !== null)
        @include($chipsView, ['chips' => $records, 'maxVisible' => count($records)])
    @else
        <div class="flex flex-wrap gap-x-4 gap-y-2">
            @forelse ($records as $record)
                ...
                @endif
            @empty
                <span class="text-gray-400 dark:text-gray-500">{{ __('custom-fields::custom-fields.record.no_records') }}</span>
            @endforelse
        </div>
    @endif
</x-dynamic-component>
